<?php 
class cCargos extends mCargos {
	
	function descripcionInsert($var) {
		$sql = "SELECT cadescri FROM amcargos WHERE cadescri=?";
		$res = $this->con->prepare($sql);
		$res->bindParam(1,$var);
		$res->execute();
		if($res->rowCount()>0) {
			$rt = "Cargo ya registrado";
		} else {
			$rt = 1;
		}
		return $rt;
	...
	function descripcionUpdate($var,$ide) {
		$sql = "SELECT cadescri FROM amcargos WHERE cadescri=? AND caide!=?";
		$res = $this->con->prepare($sql);
		$res->bindParam(1,$var);
		$res->bindParam(2,$ide);
		$res->execute();
	...
	function abreviaturaInsert($var) {
		$sql = "SELECT caabrevi FROM amcargos WHERE caabrevi=?";
		$res = $this->con->prepare($sql);
		$res->bindParam(1,$var);
		$res->execute();
		if($res->rowCount()>0) {
			$rt = "Abreviatura de cargo ya registrada";
		} else {
			$rt = 1;
		}
		return $rt;
	...
	function abreviaturaUpdate($var,$ide) {
		$sql = "SELECT caabrevi FROM amcargos WHERE caabrevi=? AND caide!=?";
		$res = $this->con->prepare($sql);
		$res->bindParam(1,$var);
		$res->bindParam(2,$ide);
		$res->execute();
